...
A perfect maze generator that is parallelized two ways by domain decomposition with an OpenMP shared-memory version and an MPI distributed-memory version.
...
printed. The sequential baseline is simply a run with one worker.

## Layout

    src/        maze.c, maze.h   sequential carving an ASCII printing
                maze_omp.c        OpenMP driver
                maze_mpi.c        MPI driver
    scripts/    strong.sh, weak.sh    scaling sweeps
                multinode.sh          same ranks on 1 vs 2 nodes (MPI only)
                common.sh             shared helpe
                job_omp.pbs, job_mpi.pbs   PBS job scripts
    results/    example_maze.txt and raw timing logs
    plots/      summary CSVs
...
Run the sweeps from the repository root:
    ...
    ./scripts/strong.sh 4000 4000
    ./scripts/weak.sh   4000 4000

Each setting is run `RUNS` times and reported as the mean and sample standard
deviation. In `weak.sh` the width is fixed and the height is rows-per-worker times the worker count.
...
1. **Connect.** If you are not on campus, open the university VPN. Then SSH in
   with your credentials:
        ...
        ssh username@hpc3-login.unitn.it

2. **Copy the project**:

        scp -r maze-generator username@hpc3-login.unitn.it:~/
...
4. **Node core count** (already assumed 64). Verify with:
   ...
   `WORKERS` list to match, and adjust `mpiprocs`/`select` in `job_mpi.pbs` so that the
   run still totals 128 ranks across the nodes.

5. **Submit from the repository root**:

        cd ~/maze-generator
        qsub scripts/job_omp.pbs     # OpenMP strong and weak, 1 node, 1..64
        qsub scripts/job_mpi.pbs     # MPI strong and weak, 2 nodes, 1..128
        qstat -u username            # see jobs

   The MPI job requests two nodes and may queue for a while.
   
7. **Collect results** from `results/maze_omp.o` / `results/maze_mpi.o`:

        scp 'username@hpc3-login.unitn.it:~/maze-generator/results/maze_*.[oe]' ./results/

   `maze_mpi.o` starts with a node-placement banner, then the `# strong scaling …` and `# weak scaling …`
